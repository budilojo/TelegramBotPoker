...
/**
 * Детерминированный ГСЧ для тестов (mulberry32). Возвращает `rnd(n)` в том же
 * формате, что ждёт `shuffle`.
 *
 * Не «s * 1103515245 + 12345» в обычных числах JS: произведение вылезает за
 * 2^53, младшие биты теряются, и генератор за ~4000 шагов сваливается в цикл
 * длиной 419 с девятью разными младшими байтами. На таком генераторе
 * «3000 случайных раздач» — это пара сотен одних и тех же, и тест зеленеет
 * вхолостую. `Math.imul` умножает честно в 32 битах.
 */
